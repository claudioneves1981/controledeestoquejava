...
public class Historico implements Initializable {
    ...
    @FXML
    private ComboBox<String> cbFiltroCompra;
    ...
    public void show() throws IOException {
        ...
        root.setControllerFactory(c -> new Historico());
    ...
    public void showVenda(boolean venda) throws IOException {
        Stage primaryStage = new Stage();
        FXMLLoader root = new FXMLLoader(getClass().getResource("/views/Historico.fxml"));
        root.setControllerFactory(c -> new Historico(venda));
        primaryStage.setTitle("ControlX - Histórico");
        Main.stage.hide();
        Main.stage = primaryStage;
        primaryStage.setScene(new Scene(root.load(), primaryStage.getWidth(), primaryStage.getHeight()));
    ...
    public void verificaSelecao(){
        btDetalhesCompra.setDisable(tbCompras.getSelectionModel().isEmpty());

        btDetalhesVenda.setDisable(tbVendas.getSelectionModel().isEmpty());
    }
    ...
    public void checaFiltroCompra() throws ClassNotFoundException {
        ...
        if(Objects.equals(cbFiltroCompra.getValue(), "Hoje")){
            fillCompraByDate(c.getTime());
        }
        else if (Objects.equals(cbFiltroCompra.getValue(), "Ultimos 7 dias")){
            c.add(Calendar.DATE, -7);
            fillCompraByDate(c.getTime());
        }
        else if (Objects.equals(cbFiltroCompra.getValue(), "Ultimo mês")){
    ...
    public void checaFiltroVenda() throws ClassNotFoundException {
        ...
        if(Objects.equals(cbFiltro.getValue(), "Hoje")){
            fillVendaByDate(c.getTime());
        }
        else if (Objects.equals(cbFiltro.getValue(), "Ultimos 7 dias")){
            c.add(Calendar.DATE, -7);
            fillVendaByDate(c.getTime());
        }
        else if (Objects.equals(cbFiltro.getValue(), "Ultimo mês")){
            c.add(Calendar.DATE, -30);
            fillVendaByDate(c.getTime());
        }
        else
